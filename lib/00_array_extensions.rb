...
class Array
  def my_uniq
    self.reduce([]) do |acc,curr|
      acc << curr if !acc.include?(curr)
      acc
    end
  end
end
...
class Array
  def two_sum
    result = []
    self.each_with_index do |el, lft_idx|
      rgt_idx = lft_idx + 1
      while rgt_idx < self.length
        result << [lft_idx, rgt_idx] if self[rgt_idx] + el == 0
        rgt_idx += 1
      end
    end
    result
...
class Array
  def median
    ...
    arr = self.sort
    lft_idx = ((arr.length.to_f / 2) - 0.5).floor
    rgt_idx = ((arr.length.to_f / 2) - 0.5).ceil
    ((arr[lft_idx].to_f + arr[rgt_idx].to_f) / 2)
  end
end
...
class Array
  def my_transpose
    result = []
    self[0].length.times do |col_idx|    
      result.push([])
      self.length.times do |row_idx|
        result[col_idx].push(self[row_idx][col_idx])
      end
    end
    result
  end
